...
STAGES = [
    ("layer0",    "Layer 0  日志标准化",           "new_data/layer0_output"),
    ("layer1",    "Layer 1  LLM 会话标注",         "new_data/layer1_output"),
    ("layer2",    "Layer 2  经验蒸馏",            "new_data/layer2_output"),
    ("layer3_p12","Layer 3  Phase 1+2  SEC/EWC",   "new_data/layer3_output/phase12_result.jsonl"),
    ("layer3_p34","Layer 3  Phase 3+4  RME/BCC",   "new_data/layer3_output/phase34_consolidated.jsonl"),
    ("layer3_p5", "Layer 3  Phase 5    KLM",        "new_data/layer3_output/phase5_klm_registry.jsonl"),
    ("layer4",    "Layer 4  缺口感知 + 冲突检测",  "new_data/layer4_output/gap_dispatch_summary.json"),
    ("upload",    "Upload   上传 Layer3 融合经验到 RAGflow", "new_data/layer3_output/phase34_consolidated.jsonl"),
]
STAGE_KEYS   = [s[0] for s in STAGES]
STAGE_LABELS = {s[0]: s[1] for s in STAGES}

_STATE_FILE = ROOT / "new_data" / ".pipeline_state.json"
...
def _build_run_cmd(
    ...
    parts = [python, "run/run_inc_pipeline.py"]
    if stages:
        parts += ["--stages"] + stages
    if no_ragflow:
