#!/usr/bin/env python3
"""
run/run_layer3_phase34.py
=========================
Layer 3 Phase 3+4 入口：RME 融合 + BCC 校准。
...
logger = logging.getLogger("run_layer3_phase34")


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="执行 Layer3 Phase3+4 (RME+BCC)")
    parser.add_argument(
        "--input-file",
        type=Path,
...
def main() -> int:
    ...
    if dropped_dups:
        logger.warning(
            "检测到重复 exp_id=%d 条，已在 Phase34 入口去重（原始=%d，去重后=%d）",
            dropped_dups,
            len(experiences_raw),
            len(experiences),
        )
    ...
    if filled_count:
        logger.info("Phase34 入口: 按 session 回填 target_service=%d 条", filled_count)
    ...
    rows = [asdict(ce) for ce in consolidated_exps]
    rows, dropped_out_dups = _dedupe_by_exp_id(rows)
    if dropped_out_dups:
        logger.warning(
            "检测到重复 consolidated exp_id=%d 条，已在 Phase34 输出端去重（BCC产出=%d，写出=%d）",
            dropped_out_dups,
            len(consolidated_exps),
            len(rows),
        )
    ...
    logger.info(
        "Phase3+4 完成: input=%d clusters=%d merges=%d bcc=%d consolidated=%d output=%s elapsed=%.2fs",
        len(experiences),
        len(clusters),
        len(merge_results),
        len(bcc_results),
