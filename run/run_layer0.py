...
用法示例：
    ...
    # 处理单个日志文件
    python run/run_layer0.py --file logs/cai_xxx.jsonl --output-dir data/processed

    # 批量处理整个目录
    python run/run_layer0.py --log-dir logs --output-dir data/processed

    # 自定义配置文件
    python run/run_layer0.py --log-dir logs --config configs/my_config.yaml

    # 仅输出统计信息，不保存文件
    python run/run_layer0.py --log-dir logs --dry-run

    # 输出 JSON 摘要
    python run/run_layer0.py --log-dir logs --output-dir out/ --summary summary.json
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="run_layer0",
        description="LORE Layer 0 流水线：将原始 JSONL 日志转换为 TurnSequence",
    ...
    p.add_argument(
        ...
        default=None,
        metavar="CONFIG_FILE",
        help="用户配置路径（默认: configs/config.yaml；设计参数自动从 configs/design.yaml 加载）",
    )
