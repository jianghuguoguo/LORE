...
def _normalize_failure_root_cause_payload(raw_value):
    """将 failure_root_cause 规范化为 JSON 对象（兼容历史字符串化数据）。"""
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            try:
                # 兼容旧数据："{'dimension': 'DEF', ...}" 这种 Python repr 字符串
    ...
    normalized = {
        "dimension": dimension,
        "sub_dimension": str(value.get("sub_dimension", "")).strip(),
        "evidence": str(value.get("evidence", "")).strip(),
        "source": str(value.get("source", "rule")).strip() or "rule",
        "remediation_hint": value.get("remediation_hint"),
        "reasoning": value.get("reasoning"),
    }

    search_queries = value.get("search_queries")
    if isinstance(search_queries, list):
        normalized["search_queries"] = [str(x) for x in search_queries if str(x).strip()]
    elif search_queries is None:
        normalized["search_queries"] = []
    else:
        normalized["search_queries"] = [str(search_queries)]
...
def save_annotated_turn_sequence(ann_seq: AnnotatedTurnSequence, output_path: Path) -> None:
    ...
    d = _to_dict(ann_seq)
    for ev in d.get("annotated_events", []):
        ev["failure_root_cause"] = _normalize_failure_root_cause_payload(
            ev.get("failure_root_cause")
        )
...
def load_annotated_turn_sequence(input_path: Path) -> AnnotatedTurnSequence:
    ...
    def _deser_failure(d) -> Optional[FailureRootCause]:
        d = _normalize_failure_root_cause_payload(d)
        if d is None:
            return None
        try:
            dim = FailureRootCauseDimension(str(d.get("dimension", "")).upper())
        except ValueError:
            return None
