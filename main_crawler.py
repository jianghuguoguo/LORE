"""
多源爬虫系统 - 主入口文件（交互式版本）
使用模块化爬虫架构

使用方法:
    python main_crawler.py              # 交互式选择数据源
    python main_crawler.py --all        # 爬取所有数据源
    python main_crawler.py --sources csdn,github  # 指定数据源
"""

import argparse
from crawlers.crawler_manager import CrawlerManager
from crawlers.config import ENABLED_SOURCES


def select_sources_interactive(available_sources: list) -> list:
    """交互式选择数据源"""
    print("\n" + "="*80)
    print("📋 可用的数据源:")
    print("="*80)
    
    for i, source in enumerate(available_sources, 1):
        status = "✓ 已启用" if ENABLED_SOURCES.get(source, False) else "✗ 未启用"
        print(f"{i}. {source.upper():15s} {status}")
    
    print(f"{len(available_sources) + 1}. 全部数据源")
    print("="*80)
    
    while True:
        choice = input("\n请选择要爬取的数据源 (输入数字，多个用逗号分隔，或 'q' 退出): ").strip()
        
        if choice.lower() == 'q':
            print("退出程序")
            return []
        
        try:
            # 解析输入
            choices = [int(x.strip()) for x in choice.split(',')]
            
            selected = []
            for num in choices:
                if num == len(available_sources) + 1:
                    # 选择全部
                    return available_sources
                elif 1 <= num <= len(available_sources):
                    source = available_sources[num - 1]
                    if source not in selected:
                        selected.append(source)
                else:
                    print(f"⚠️  无效选项: {num}")
                    continue
            
            if selected:
                return selected
            else:
                print("⚠️  请至少选择一个数据源")
        
        except ValueError:
            print("⚠️  输入格式错误，请输入数字（如: 1,2,3）")


def get_search_query() -> tuple:
    """获取搜索关键词和参数"""
    print("\n" + "="*80)
    print("设置搜索参数")
    print("="*80)
    
    # 获取搜索关键词（可选）
    query = input("请输入搜索关键词: ").strip()
    
    # 如果没有关键词，给出提示
    if not query:
        print("未指定关键词，将爬取数据源的所有内容")
    
    # 获取CSDN页数
    while True:
        try:
            max_pages_input = input("最大爬取页数: ").strip()
            max_pages = int(max_pages_input) if max_pages_input else 5
            if max_pages > 0:
                break
            print("⚠️  页数必须大于0")
        except ValueError:
            print("⚠️  请输入有效数字")
    
    # 输出目录
    output_dir_input = input("输出目录 (默认raw_data, 按回车使用默认值): ").strip()
    output_dir = output_dir_input if output_dir_input else 'raw_data'
    
    return query, max_pages, output_dir


def main():
    """主函数 - 交互式爬虫"""
    
    # 解析命令行参数
    parser = argparse.ArgumentParser(
        description="多源爬虫系统",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 交互式选择数据源
  python main_crawler.py
  
  # 爬取所有数据源
  python main_crawler.py --all -q CVE-2024-23897
  
  # 指定数据源
  python main_crawler.py --sources csdn,github -q CVE-2024-23897 --max-pages 10
  
  # 快速模式（跳过确认）
  python main_crawler.py --all -q CVE-2024-23897 --yes
        """
    )
    
    parser.add_argument('--all', action='store_true', help='爬取所有已启用的数据源')
    parser.add_argument('--sources', type=str, help='指定数据源，用逗号分隔 (如: csdn,github,attack)')
    parser.add_argument('-q', '--query', type=str, help='搜索关键词')
    parser.add_argument('--max-pages', type=int, default=5, help='最大爬取页数 (默认5)')
    parser.add_argument('-o', '--output', default='multi_source_output', help='输出目录 (默认multi_source_output)')
    parser.add_argument('-y', '--yes', action='store_true', help='跳过确认提示')
    
    args = parser.parse_args()
    
    # 创建爬虫管理器
    manager = CrawlerManager()
    available_sources = manager.list_crawlers()
    
    if not available_sources:
        print("❌ 错误: 没有可用的爬虫")
        return
    
    print("\n" + "="*80)
    print("🚀 多源爬虫系统")
    print("="*80)
    print(f"已注册的爬虫: {', '.join(available_sources)}")
    print("="*80)
    
    # 确定要使用的数据源
    if args.all:
        selected_sources = available_sources
        print(f"\n✓ 已选择所有数据源: {', '.join(selected_sources)}")
    elif args.sources:
        # 从命令行参数解析
        requested = [s.strip().lower() for s in args.sources.split(',')]
        selected_sources = [s for s in requested if s in available_sources]
        
        if not selected_sources:
            print(f"❌ 错误: 指定的数据源不可用")
            print(f"可用数据源: {', '.join(available_sources)}")
            return
        
        invalid = [s for s in requested if s not in available_sources]
        if invalid:
            print(f"⚠️  警告: 以下数据源不可用: {', '.join(invalid)}")
        
        print(f"\n✓ 已选择数据源: {', '.join(selected_sources)}")
    else:
        # 交互式选择
        selected_sources = select_sources_interactive(available_sources)
        if not selected_sources:
            return
        
        print(f"\n✓ 已选择: {', '.join(selected_sources)}")
    
    # 获取搜索参数
    if args.query is not None:  # 允许空字符串
        query = args.query
        max_pages = args.max_pages
        output_dir = args.output
        if query:
            print(f"\n 搜索关键词: {query}")
        else:
            print(f"\n 无关键词模式: 爬取所有内容")
        print(f" 最大爬取页数: {max_pages}")
        print(f"📁 输出目录: {output_dir}")
    else:
        query, max_pages, output_dir = get_search_query()
    
    # 确认执行
    if not args.yes:
        print("\n" + "="*80)
        print("爬取任务配置:")
        print("="*80)
        print(f"数据源: {', '.join(selected_sources)}")
        if query:
            print(f"关键词: {query}")
        else:
            print(f"关键词: (无，爬取所有内容)")
        print(f"最大页数: {max_pages}")
        print(f"输出目录: {output_dir}")
        print("="*80)
        
        confirm = input("\n是否开始爬取? (y/n): ").strip().lower()
        if confirm != 'y':
            print("已取消")
            return
    
    # 执行爬取
    print("\n" + "="*80)
    print("🚀 开始爬取...")
    print("="*80)
    
    try:
        # 只爬取选中的数据源
        results = {}
        for source in selected_sources:
            print(f"\n>>> 爬取 {source.upper()} ...")
            result = manager.crawl_single_source(
                source,
                query,
                max_pages=max_pages # 传递页数参数
            )
            
            if result:
                results[source] = result
                print(f"✓ {source.upper()}: {len(result)} 条记录")
            else:
                print(f"✗ {source.upper()}: 无数据")
        
        # 保存结果
        if results:
            print(f"\n💾 保存结果到: {output_dir}")
            manager.save_results(results, query, output_dir=output_dir)
            manager._print_summary(results)
            
            print("\n" + "="*80)
            print("✅ 爬取任务完成！")
            print("="*80)
            print(f"📁 结果保存在: {output_dir}")
            print(f"📊 总计: {sum(len(v) for v in results.values())} 条记录")
        else:
            print("\n⚠️  没有获取到任何数据")
    
    except KeyboardInterrupt:
        print("\n\n⚠️  用户中断")
    except Exception as e:
        print(f"\n❌ 错误: {e}")
        import traceback
        traceback.print_exc()


if __name__ == "__main__":
    main()
