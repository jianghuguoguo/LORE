...
# 从头重置状态再执行
python run/run_full_pipeline.py --reset-state
```
...
# Layer 3：XPEC 融合
python run/run_layer3_phase12.py   # Phase 1+2：SEC + EWC
python run/run_layer3_phase34.py   # Phase 3+4：RME + BCC
python run/run_layer3_phase5.py    # Phase 5：KLM 注册

# Layer 4：缺口感知
python run/run_layer4_gap_dispatch.py --no-crawl   # 仅分析，不触发爬取
...
| 知识层 | 路由键 | 说明 |
|:------|:------|:----|
| `FACTUAL` | `dataset_factual` | 事实知识库 |
| `PROCEDURAL_POS` | `dataset_procedural_pos` | 成功步骤库 |
| `PROCEDURAL_NEG` | `dataset_procedural_neg` | 失败避坑库 |
...
> 结论：**METACOGNITIVE 与 CONCEPTUAL 默认同库上传**；
> `full_dataset` 仅作未来归档/实验用途，不参与当前生产检索路由。

仅补传 CONCEPTUAL 子集（示例）：

```bash
python src/ragflow_uploader.py --source fused --exp-ids exp_a,exp_b,exp_c --retry-502-max 8 --retry-base-sec 2.0
```

Layer 3 判定规则说明：

- 融合阈值按知识层生效：`CONCEPTUAL >= 2`，其余层保持 `>= 3`。
- `conflicted` 判定为“矛盾度超过层级阈值且 maturity 非 consolidated”。
- 冲突阈值分层：`CONCEPTUAL/METACOGNITIVE = 0.30`，其他层为 `0.60`。
...
避免重复的正确姿势：始终使用 `run/run_full_pipeline.py`（末尾统一上传），
...
| 频次 | 任务 |
|:-----|:----|
| 每次新增日志 | `python run/run_full_pipeline.py`（全量重算） |
| 每周 | 更新 CSDN Cookie · 清理 raw_data/ 重复文件 |
| 按需 | `python crawlers/sync_data_light.py --repos attack,cve` |
| 离线调试 | `python run/run_full_pipeline.py --no-ragflow` |
